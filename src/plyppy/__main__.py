...
def template_plymouth(name):
    bg_start = Prompt.ask(
        "[blue]Enter the hex code of the top of the background: ", default="000000"
    )
    bg_end = Prompt.ask(
        "[blue]Enter the hex code of the bottom of the background: ", default="2a2a2a"
    )
        ...
        while True:
            try:
                ...
                output_frame_path = os.path.join(
                    output_dir, f"throbber-{frame_index:04d}.png"
                )
...
def validate_file_path(file_path):
    ...
    if not abs_path or "\0" in abs_path:
        raise InvalidResponse(
            "[bold red]Error: Invalid file path. Please ensure the file path is correct and does not contain null characters.[/]"
        )
...
def install_plymouth_theme(output_dir):
    name = os.path.basename(os.path.normpath(output_dir))
    subprocess.run(
        ["sudo", "cp", "-r", output_dir, f"/usr/share/plymouth/themes/{name}"]
    )
...
def main():
    ...
    while True:
        output_dir = Prompt.ask(
            "[blue]Enter the output directory for saving frames (also the name of the theme)"
        )
    ...
    console.log("Enjoy your custom theme!")
